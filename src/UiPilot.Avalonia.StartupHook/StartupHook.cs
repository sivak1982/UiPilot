...
/// can load beside the app's shared-framework assemblies, while Avalonia types stay shared
/// with the target app.
...
internal static class StartupHook
{
    ...
    public static void Initialize()
    {
        ...
        {
            IsBackground = true,
            Name = "UiPilot.Avalonia.StartupHook",
        };
        thread.Start();
    }
    ...
    private static void WaitAndStart()
    {
        for (var i = 0; i < MaxAttempts; i++)
        {
            try
            {
                if (TryGetAvaloniaMainWindow() != null)
                {
                    StartPilot();
                    return;
                }
    ...
    private static object? TryGetAvaloniaApplicationCurrent()
    {
        var appType = FindLoadedType("Avalonia.Application");
        if (appType is null)
            return null;
    ...
    {
        var app = TryGetAvaloniaApplicationCurrent();
        if (app is null)
            return null;
        ...
        return lifetime.GetType().GetProperty("MainWindow")?.GetValue(lifetime);
    }
    ...
    private static void StartPilot()
    {
        try
        {
            ...
            var pilotPath = Path.Combine(hookDir, "UiPilot.Avalonia.dll");
            ...
            var alc = new PilotLoadContext(hookDir, shareUiFramework: "Avalonia");
            var asm = alc.LoadFromAssemblyPath(pilotPath);
            var hostType = asm.GetType("UiPilot.Avalonia.PilotHost", throwOnError: true)!;
    ...
    {
        Debug.WriteLine("[UiPilot.Avalonia.StartupHook] " + message);
        try
        {
            var path = Path.Combine(Path.GetTempPath(), "uipilot-startup-hook.log");
            File.AppendAllText(path, DateTime.UtcNow.ToString("o") + " avalonia " + message + Environment.NewLine);
        }
        catch
        {
            // ignore
    ...
    /// <summary>
    /// Loads UiPilot payload privately; resolves Avalonia (or WPF) assemblies from the default context.
    /// </summary>
    private sealed class PilotLoadContext : AssemblyLoadContext
    {
        private readonly string _hookDir;
        private readonly AssemblyDependencyResolver _resolver;
        private readonly string _sharePrefix;

        public PilotLoadContext(string hookDir, string shareUiFramework)
            : base("UiPilot." + shareUiFramework, isCollectible: false)
        {
            _hookDir = hookDir;
            _sharePrefix = shareUiFramework;
            var depsAnchor = Path.Combine(hookDir, "UiPilot." + shareUiFramework + ".dll");
            _resolver = new AssemblyDependencyResolver(depsAnchor);
        }
        ...
        private bool ShouldShareWithApp(string simple) =>
            simple.StartsWith(_sharePrefix, StringComparison.OrdinalIgnoreCase)
            || simple.StartsWith("MicroCom", StringComparison.OrdinalIgnoreCase)
            || simple.StartsWith("HarfBuzz", StringComparison.OrdinalIgnoreCase)
            || simple.StartsWith("SkiaSharp", StringComparison.OrdinalIgnoreCase)
            || simple.StartsWith("Tmds.", StringComparison.OrdinalIgnoreCase);
    }
}
