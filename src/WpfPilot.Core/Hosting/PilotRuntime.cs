...
namespace WpfPilot.Hosting;

/// <summary>
/// Shared start/stop wiring for any UI framework backend: tool registry, named pipe, discovery file.
/// Framework hosts (<c>WpfPilotHost</c>, <c>AvaloniaPilotHost</c>) stay thin wrappers around this.
/// </summary>
public sealed class PilotRuntime : IDisposable
{
        ...
        Action<string> log)
    {
        ...
        lock (_gate)
        {
            ...
            {
                log("Pilot disabled (not a Debug build, UIPILOT_ENABLE/WPFPILOT_ENABLE!=1, and Force=false).");
                return false;
            }
